...
class MinesweeperAI():
	...
	def add_knowledge(self, cell, count):
		...
		# undetermined cells only
		unknown = set()
		for each in cells:
			if each not in self.safes and each not in self.mines:
				unknown.add(each)
			# IMPORTANT
		...
		new = Sentence(unknown, count)
		self.knowledge.append(new)
		
		# 4,5) IMPORTANT
		while True:
			old = self.knowledge.copy()
			self.change()
			if old == self.knowledge:
				break
	...
	def change(self):
		...
		for p in self.knowledge:
			for q in self.knowledge:
				if p.cells > q.cells:
					new = Sentence(p.cells - q.cells, p.count - q.count)
					if new not in self.knowledge and new not in news:
						news.append(new)
		# IMPORTANT				
		for n in news:
